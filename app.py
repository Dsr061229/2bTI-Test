...
st.markdown("""
    ...
    .stButton>button {
        ...
        transition: 0.2s;
        text-shadow: none !important; /* 按钮文字不需要阴影 */
    }
...
if 'scores' not in st.session_state:
    st.session_state.scores = {"NB": 0, "SB": 0, "NM": 0, "TM": 0}
...
# 【测试页】
    ...
    questions = [
        {"q": "Q1：深夜独自在卧室想确认自己的硬件规格（物理），你会？", 
         "opts": [("拿卷尺精准测量并Excel建模对比", "NB"), ("敲击键盘问AI这算不算乐器", "SB")]},
        {"q": "Q2：当你在公共场合不小心产生了一个“生化武器”级闷屁时？", 
         "opts": [("保持眼神坚定，抬头挺胸帅气走开", "NB"), ("悄悄兜一下闻闻确认肠道菌群是否健康", "NM")]},
        {"q": "Q3：如果你在街上看到一个绝世美女/帅哥，内心OS是？", 
         "opts": [("他在看我，一定被我的贵族气质吸引了", "NB"), ("问他愿不愿意教我在水里吐泡泡", "SB"), ("这腿够玩一年，洗澡会搓出很多泥吧", "NM"), ("当众表演原地拉稀引起他的注意", "TM")]},
        {"q": "Q4：关于“Bra”和“Bro”的选择，你更倾向于？", 
         "opts": [("研究Bra的力学结构（理工男的严谨）", "NM"), ("把Bra戴在头上大喊：看我新买的防雷头盔！", "SB")]},
        {"q": "Q5：发现自己今天没穿内裤出门时，你会？", 
         "opts": [("视为原始自由，步伐更加坚定自信", "NB"), ("突然掀起衣角向好友展示那空荡荡的腰间", "SB"), ("以此撰写朋友圈“局部通风健康指南”", "NM"), ("在公共座椅疯狂磨蹭留下属于自己的信息", "TM")]},
        {"q": "Q6：如果你误入了一个满是异性的更衣室，你会？", 
         "opts": [("优雅转身：别看我，我只是路过的神", "NB"), ("螃蟹步逃离现场并大喊：为了部落！", "SB"), ("研究排水沟消防结构（其实耳朵竖得比兔高）", "NM"), ("表演杂技并问在场的人需不需要搓背", "TM")]}
    ]
...
# 【结果页】
    ...
    res = max(st.session_state.scores, key=st.session_state.scores.get)
    ...
    <div class="result-card">
        ...
        <p style='color: #00ffff;'><b>匹配好友：</b>建议远离正常人，寻找同类互相伤害。</p>
    </div>
    """, unsafe_allow_html=True)
